# Audio additions.
# NOTIFY: https://github.com/Twentysix26/Red-DiscordBot/pull/379
# NOTIFY as an embed with random colors: aikaterna, with 2 lines from Sentry's embedsay
# NOTIFY_CHANNEL: Axas, from forkaudio NOTIFY_CHANNEL
# Embed song and queue: FwiedRice
# Code cleanup, unload, except handling, and loop additions: Redjumpman
import asyncio
import collections
import copy
import discord
...
try:
    ...
    opus = False
except FileNotFoundError:  # Missing opus
    opus = None
else:
    opus = True
...
class Audio:
    ...
    def __init__(self, bot, player):
        ...
        self.server_specific_setting_keys = ["VOLUME", "VOTE_ENABLED",
                                             ...
                                             "NOTIFY", "NOTIFY_CHANNEL"] # added notify to server-specific settings
        ...
        self.save_settings()
        # Event loops
        self.queue_task = bot.loop.create_task(self.queue_scheduler())
        self.disconnect_task = bot.loop.create_task(self.disconnect_timer())
        self.reload_task = bot.loop.create_task(self.reload_monitor())
        self.cache_task = bot.loop.create_task(self.cache_scheduler())
    ...
    async def _notify_song_name(self):
        for sid in self.bot.servers:
            settings = self.get_server_settings(sid.id)
            notif = settings.get("NOTIFY", True)
            if notif:
                song = self.queue[sid.id]["NOW_PLAYING"]
                color = int(''.join([random.choice('0123456789ABCDEF') for x in range(6)]), 16)
                channel = self.bot.get_channel(self.get_server_settings[sid.id]["NOTIFY_CHANNEL"])
                if channel is None:
                ...
                if song:
                    ...
                    await self.bot.send_message(channel, "I'm not playing anything right now.")

            elif "NOTIFY" not in self.get_server_settings(sid.id):
                self.get_server_settings[sid.id]["NOTIFY"] = False
                self.save_settings()
    ...
    async def audioset_notify(self, ctx):
        """Toggles sending a message when the song changes"""
        server = ctx.message.server
        settings = self.get_server_settings(server.id)
        notify = settings.get("NOTIFY", True) # this needs to be changed to be the server-specific block of the json instead of global, which it is now
    ...
    @audioset.command(name="notify_channel", pass_context=True)
    @checks.mod_or_permissions(manage_messages=True)
    async def notify_channel(self, ctx, channel:str=None):
        """Set the channel id for track notifications"""
        server = ctx.message.server
        if channel is None:
            self.get_server_settings(server.id)["NOTIFY_CHANNEL"] = ctx.message.channel.id
        else:
            self.get_server_settings(server.id)["NOTIFY_CHANNEL"] = self.bot.get_channel(channel).id # this needs to be changed to accept channel names instead of just id's
        channel = self.get_server_settings(server)["NOTIFY_CHANNEL"]
        await self.bot.say("Notify channel successfully set as {}.".format(self.bot.get_channel(channel).mention))
        if not self.get_server_settings(server.id)["NOTIFY_CHANNEL"]:
    ...
    async def play(self, ctx, *, url_or_search_terms):
        ...
        author = ctx.message.author
        voice_channel = author.voice_channel
        notify = self.get_server_settings(server)["NOTIFY_CHANNEL"] # this line currently throws an error, I'm trying to find a way to get the server id that's stored via audioset notify_channel
        self.playing_text_channel = notify
    ...
    async def playlist_start(self, ctx, name):
        ...
        self.playing_text_channel = self.get_server_settings(server)["NOTIFY_CHANNEL"] # this line currently throws an error, I'm trying to find a way to get the server id that's stored via audioset notify_channel
    ...
    async def _queue_list(self, ctx):
        ...
        if now_playing is not None:
            data = discord.Embed(description="Queue".format(now_playing.title),
                                 color=discord.Colour(value=color))
            data.add_field(name="***Now playing:***", value=now_playing.title)
        ...
        data.add_field(name="***Next up:***", value="\n".join(song_info))
    ...
    async def song(self, ctx):
        ...
        if song:
            ...
            color = ''.join([choice('0123456789ABCDEF') for x in range(6)])
            color = int(color, 16)
            data = discord.Embed(description="Song Info", colour=discord.Colour(value=color))
            data.add_field(name="**Title**", value=song.title)
            data.add_field(name="**Youtube Channel**", value=song.uploader)
            data.add_field(name="**Views**", value=song.view_count)
            data.add_field(name="**Duration**", value=dur)
            data.add_field(name="Link", value=song.webpage_url)
            await self.bot.say(embed=data)
        else:
