...
class Owner:
    ...
    @commands.group(name="set", pass_context=True, hidden=True)
    async def _set(self, ctx):
        """Changes Brooklyn's global settings."""
    ...
    async def prefix(self, ctx, *prefixes):
        """Sets Brooklyn's global prefixes
    ...
    async def serverprefix(self, ctx, *prefixes):
        """Sets Brooklyn's prefixes for this server
    ...
    async def stream(self, ctx, streamer=None, *, stream_title=None):
        """Sets Brooklyn's streaming status
    ...
    @commands.group(name="command", pass_context=True, hidden=True)
    @checks.is_owner()
    ...
    @commands.command(hidden=True)
    async def invite(self, invite_url: discord.Invite=None):
        """Joins new server"""
    ...
    async def uptime(self):
        ...
        msg = "**%d weeks," % (week) + " %d days," % (day) + " %d hours," % (hour) + " %d minutes," % (minute) + " and %d seconds.**" % (second)
        data= discord.Embed(description="{}".format(msg), colour=discord.Colour(value=colour))
